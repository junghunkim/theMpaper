...
colvec CLP::getMeans(int maxMC){
  ...
  return(join_cols(retSta,retSca));
}
...
mat CLP::rejSimulator(){
  ...
  retOBJ(span::all,0) = Param(span(0,nvertex-1),0);
  ...
  for(int itr_message=0; itr_message < nmessages-1; itr_message++){
    ...
    if(itr_message == 0){
      myINIT_cur = Param(span(0,nvertex-1),0);
    } else {
	...
	for(int j = (i+1); j < nvertex; ++j){ //iterate through diff vertex
	  ...
	  mystop_prob = mystop_prob - Param(nvertex,0)*mean(path_ij_rate_sum)*(myRHS_cur-myLHS_cur);
	} // j>i for-ends
      } // i for-ends
      //
