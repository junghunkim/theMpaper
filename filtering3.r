...
myfcnC1 <- function(b,s,g,C1=1:nvertex) {
	  if(length(C1) == 0) 
		return(0)
...
#          print(COEF)
          retVal =0
      retVal = retVal + -(b/s^2 + 1/2) * COEF[1] + 
      b*(1-2*g)/(2*s^2) * COEF[2]
 #         print(retVal)
          retVal = retVal +  (-0.5)*( (-b/s + s/2)^2*(tRHS-tLHS)*length(C1) +
...
myobjfcn <- function(par) {
  -myfcnC1(par[1],1,par[2])
}
