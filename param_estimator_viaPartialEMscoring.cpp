...
int main() {
  ...
  rowvec normal_vertex_param(NPARAM);
  rowvec abnorm_vertex_param(NPARAM);
  mat myParam(NVERTEX,NPARAM);

  normal_vertex_param << 0.5 << -1 << 1 << endr;
  abnorm_vertex_param << 0.5 << -1 << 1 << endr;
  
  for(int i = 0; i < NVERTEX_Normal; i++){
    myParam.row(i) = normal_vertex_param;
  }
  
  for(int j = 0; j < NVERTEX_Abnorm; j++){
    myParam.row(NVERTEX_Normal+j) = abnorm_vertex_param;
  }
